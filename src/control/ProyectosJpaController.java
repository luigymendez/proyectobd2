...
public class ProyectosJpaController implements Serializable {
    ...
    public void create(Proyectos proyectos) throws PreexistingEntityException, Exception {
        ...
        if (proyectos.getHorariosCollection() == null) {
            proyectos.setHorariosCollection(new ArrayList<Horarios>());
        }
        if (proyectos.getEntregasCollection() == null) {
            proyectos.setEntregasCollection(new ArrayList<Entregas>());
        }
        if (proyectos.getFichasCollection() == null) {
            proyectos.setFichasCollection(new ArrayList<Fichas>());
        }
        ...
        try {
            ...
            for (Horarios horariosCollectionHorariosToAttach : proyectos.getHorariosCollection()) {
            ...
            proyectos.setHorariosCollection(attachedHorariosCollection);
            ...
            for (Fichas fichasCollectionFichasToAttach : proyectos.getFichasCollection()) {
            ...
            proyectos.setFichasCollection(attachedFichasCollection);
            ...
            for (Horarios horariosCollectionHorarios : proyectos.getHorariosCollection()) {
                Proyectos oldProyectosIdOfHorariosCollectionHorarios = horariosCollectionHorarios.getProyectosId();
                horariosCollectionHorarios.setProyectosId(proyectos);
                horariosCollectionHorarios = em.merge(horariosCollectionHorarios);
                if (oldProyectosIdOfHorariosCollectionHorarios != null) {
                    oldProyectosIdOfHorariosCollectionHorarios.getHorariosCollection().remove(horariosCollectionHorarios);
                    oldProyectosIdOfHorariosCollectionHorarios = em.merge(oldProyectosIdOfHorariosCollectionHorarios);
                }
            }
            ...
            for (Fichas fichasCollectionFichas : proyectos.getFichasCollection()) {
                Proyectos oldProyectosIdOfFichasCollectionFichas = fichasCollectionFichas.getProyectosId();
                fichasCollectionFichas.setProyectosId(proyectos);
                fichasCollectionFichas = em.merge(fichasCollectionFichas);
                if (oldProyectosIdOfFichasCollectionFichas != null) {
                    oldProyectosIdOfFichasCollectionFichas.getFichasCollection().remove(fichasCollectionFichas);
                    oldProyectosIdOfFichasCollectionFichas = em.merge(oldProyectosIdOfFichasCollectionFichas);
                }
            }
    ...
    public void edit(Proyectos proyectos) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            ...
            Collection<Horarios> horariosCollectionOld = persistentProyectos.getHorariosCollection();
            Collection<Horarios> horariosCollectionNew = proyectos.getHorariosCollection();
            ...
            Collection<Fichas> fichasCollectionOld = persistentProyectos.getFichasCollection();
            Collection<Fichas> fichasCollectionNew = proyectos.getFichasCollection();
            ...
            for (Horarios horariosCollectionOldHorarios : horariosCollectionOld) {
                if (!horariosCollectionNew.contains(horariosCollectionOldHorarios)) {
                    ...
                    illegalOrphanMessages.add("You must retain Horarios " + horariosCollectionOldHorarios + " since its proyectosId field is not nullable.");
                }
            }
            ...
            for (Fichas fichasCollectionOldFichas : fichasCollectionOld) {
                if (!fichasCollectionNew.contains(fichasCollectionOldFichas)) {
                    ...
                    illegalOrphanMessages.add("You must retain Fichas " + fichasCollectionOldFichas + " since its proyectosId field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            proyectos.setHorariosCollection(horariosCollectionNew);
            ...
            proyectos.setFichasCollection(fichasCollectionNew);
            ...
            for (Horarios horariosCollectionNewHorarios : horariosCollectionNew) {
                if (!horariosCollectionOld.contains(horariosCollectionNewHorarios)) {
                    Proyectos oldProyectosIdOfHorariosCollectionNewHorarios = horariosCollectionNewHorarios.getProyectosId();
                    horariosCollectionNewHorarios.setProyectosId(proyectos);
                    horariosCollectionNewHorarios = em.merge(horariosCollectionNewHorarios);
                    if (oldProyectosIdOfHorariosCollectionNewHorarios != null && !oldProyectosIdOfHorariosCollectionNewHorarios.equals(proyectos)) {
                        oldProyectosIdOfHorariosCollectionNewHorarios.getHorariosCollection().remove(horariosCollectionNewHorarios);
                        oldProyectosIdOfHorariosCollectionNewHorarios = em.merge(oldProyectosIdOfHorariosCollectionNewHorarios);
                    }
                }
            }
            ...
            for (Fichas fichasCollectionNewFichas : fichasCollectionNew) {
                if (!fichasCollectionOld.contains(fichasCollectionNewFichas)) {
                    Proyectos oldProyectosIdOfFichasCollectionNewFichas = fichasCollectionNewFichas.getProyectosId();
                    fichasCollectionNewFichas.setProyectosId(proyectos);
                    fichasCollectionNewFichas = em.merge(fichasCollectionNewFichas);
                    if (oldProyectosIdOfFichasCollectionNewFichas != null && !oldProyectosIdOfFichasCollectionNewFichas.equals(proyectos)) {
                        oldProyectosIdOfFichasCollectionNewFichas.getFichasCollection().remove(fichasCollectionNewFichas);
                        oldProyectosIdOfFichasCollectionNewFichas = em.merge(oldProyectosIdOfFichasCollectionNewFichas);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(BigDecimal id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            Collection<Horarios> horariosCollectionOrphanCheck = proyectos.getHorariosCollection();
            for (Horarios horariosCollectionOrphanCheckHorarios : horariosCollectionOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Proyectos (" + proyectos + ") cannot be destroyed since the Horarios " + horariosCollectionOrphanCheckHorarios + " in its horariosCollection field has a non-nullable proyectosId field.");
            }
            ...
            Collection<Fichas> fichasCollectionOrphanCheck = proyectos.getFichasCollection();
            for (Fichas fichasCollectionOrphanCheckFichas : fichasCollectionOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Proyectos (" + proyectos + ") cannot be destroyed since the Fichas " + fichasCollectionOrphanCheckFichas + " in its fichasCollection field has a non-nullable proyectosId field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
